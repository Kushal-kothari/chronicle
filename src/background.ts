...
import { checkPomodoro } from './services/pomodoro';
import { Messages } from './utils/messages';
import { injectTabsRepositorySingleton } from './repository/inject-tabs-repository';
import { isInFlowState, getCurrentFlowDurationSeconds } from './composables/useFlowDetection';
import { endActiveFocusSession } from './composables/useFocusSessions';
...
Browser.runtime.onMessage.addListener(async message => {
  if (message == Messages.ClearAllData) {
    ...
    // Also clear Chronicle-specific data
    await storage.saveValue(StorageParams.GOALS, []);
    await storage.saveValue(StorageParams.FOCUS_SESSIONS, []);
    await storage.saveValue(StorageParams.ACTIVE_FOCUS_SESSION, null);
    await storage.saveValue(StorageParams.FLOW_SESSIONS, []);
    await storage.saveValue(StorageParams.CATEGORY_OVERRIDES, {});
  }
  ...
  if (message == Messages.GetFlowState) {
    return {
      isFlow: isInFlowState(),
      durationSeconds: getCurrentFlowDurationSeconds(),
    };
  }
  if (message.message == Messages.EndFocusSession) {
    await endActiveFocusSession(message.rating, message.note);
  }
});
