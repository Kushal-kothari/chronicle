...
import { isValidPage, extractHostname, ActiveTab } from './utils/url';
import { injectTabsRepositorySingleton } from './repository/inject-tabs-repository';
import { isInBlackList } from './services/block-list';
import { useBadge, BadgeColor } from './services/badge';
...
import { NotificationType, useNotification } from './composables/useNotification';
import { Messages } from './utils/messages';
import { onDomainActive } from './composables/useFlowDetection';
...
async function mainTracker(
  ...
  if (state === 'active' || isAudibleValue) {
    ...
    if (isActiveTabWasChanged(activeDomain)) {
      ...
      await onDomainActive(activeDomain);
    }
